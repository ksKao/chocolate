...
std::string Generator::getDataName(const std::string &value, const std::string &size)
{
    ...
    {
        if (data.at(i).value == value && data.at(i).size == size)
...
{
    Generator::appendOutput("sub rsp, " + std::to_string(Generator::stackUnitSize)); // move top stack pointer up
    stackSize++;
}

void Generator::decrementStack()
{
    Generator::appendOutput("add rsp, " + std::to_string(Generator::stackUnitSize)); // move top stack pointer down
    stackSize--;
}
