...
Token Lexer::parseWord()
{
    ...
    while (getIsValidWordChar(getChar()))
    {
        ...
        // if next character is not a valid character for a word, means we have already found a word.
        // we cannot detect directly by space, since `let i=10` should still be valid syntax
        if (!getIsValidWordChar(peek()))
        {
            // in this for loop, we try to match every keyword, if a match is found, means that this word is a keyword,
            ...
            // reach this case means there are no matching keywords, i.e., the word is an identifier
            advance();
...
std::vector<Token> Lexer::tokenize()
{
    ...
    i = 0; // reset index, in case need to use the same lexer object to tokenize multiple source code
